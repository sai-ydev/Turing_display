...
HAL_StatusTypeDef lcd_send_cmd(uint8_t command, uint8_t val)
{
	...
	HAL_StatusTypeDef rslt = HAL_I2C_Master_Transmit(&hi2c1, DevAddress, buf, 2, BUS_TIMEOUT);
	return rslt;
}
...
HAL_StatusTypeDef lcd_send_buffer(uint8_t* data, uint8_t length)
{
	...
	// send register address
	HAL_StatusTypeDef rslt = HAL_I2C_Master_Transmit(&hi2c1, DevAddress, buf, (length + 2), BUS_TIMEOUT);
	return rslt;
}
...
int main(void)
{
  ...
  MX_I2C1_Init();
  /* USER CODE BEGIN 2 */
  HAL_StatusTypeDef rslt = lcd_reset();
  HAL_Delay(10);
  rslt = lcd_clear();



  rslt = lcd_fill_digits(DIGIT_SEGMENTS[7]);
